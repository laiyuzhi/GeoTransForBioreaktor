...
class Bioreaktor_Detection(Dataset):
    ...
    def __init__(self, root, resize, mode, translation_x=0.25, translation_y=0.25):
        ...
        for name in sorted(os.listdir(os.path.join(self.root))):
            ...
            self.name2label[name] = [int(x) for x in re.findall(r"\d+", name)]
        #  72
    ...
    def load_csv(self, filename):
        ...
        if not (os.path.exists(os.path.join(self.root, filename))):
            ...
            for name in self.name2label.keys():
                ...
                if self.mode == 'Test' and (name == 'Speed400Luft20' or name == 'Speed0Luft2' or name == 'Speed0Luft20'
                                            or name == 'Speed600Luft0' or name == 'Speed800Luft0'):
                ...
                elif (self.mode == 'Vali' or self.mode == 'Prozess') and (name == 'Speed400Luft0' or name == 'Speed400Luft2'):
                    data_list += glob.glob(os.path.join(self.root, name, '*.png'))
                    data_list += glob.glob(os.path.join(self.root, name, '*.jpg'))
                    data_list += glob.glob(os.path.join(self.root, name, '*.jpeg'))
                elif self.mode == 'Train' and (name == 'Speed400Luft0' or name == 'Speed400Luft2'):
                    data_list += glob.glob(os.path.join(self.root, name, '*.png'))
                    data_list += glob.glob(os.path.join(self.root, name, '*.jpg'))
                    data_list += glob.glob(os.path.join(self.root, name, '*.jpeg'))
        ...
        with open(os.path.join(self.root, filename)) as f1:
                ...
                for row in reader:
                    ...
                    label = int(label)
                    multi_input = [int(x) for x in re.findall(r"\d+", multi_input)]
    ...
    def __getitem__(self, idx):
        ...
        list1 = [0., 200., 400., 600., 800.]
        list2 = [0., 2., 20.]
        if self.mode == 'Prozess':
            multi_inputs[0] = np.random.choice(list(set(list1) ^ set([multi_inputs[0]])))
            multi_inputs[0] = (multi_inputs[0] - 500 + (2 * random.gauss(0, 1))) / 374.1657
            multi_inputs[1] = np.random.choice(list(set(list2) ^ set([multi_inputs[1]])))
            multi_inputs[1] = (multi_inputs[1] - 7.3 + (random.gauss(0, 1)/10) ) / 11.0151
            multi_inputs = torch.tensor(multi_inputs).float()
        else:
            multi_inputs[0] = (multi_inputs[0] - 7.3 + (random.gauss(0, 1) * 2)) / 374.1657
            multi_inputs[1] = (multi_inputs[1] - 7.3 + (random.gauss(0, 1)/10) ) / 11.0151
            multi_inputs = torch.tensor(multi_inputs).float()
...
# print('x2:', x2, 'label:', label, x2.shape)
# for i in range(72):
#     x, y, z = next(Iter)
#     print(len(y), z)
#     # if i > 6:
